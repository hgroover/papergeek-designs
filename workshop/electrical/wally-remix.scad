...
// Modified by PaperGeek to add recessed screw option
//
...
//
//  v1.4
//  ADDED (PaperGeek): Recessed screw option
...
// Screws can't reach? Try increasing screwhole recess in mm
screwhole_recess = 0; // [0:10]
...
module plate1(){
	if (1st_plate == "toggle" || 1st_plate_bottom_hole == "toggle"){
		translate([0,l_offset[plate_size],-screwhole_recess]) toggle_screws();
...
module toggle_screws(){
     ...
     translate([height_sizes[plate_size]/2 + 30.1625,0,6.0]) cylinder(r=3.6, h=10);
	 translate([height_sizes[plate_size]/2 - 30.1625,0,-1]) cylinder(r=2, h=10, $fn=12);
	 translate([height_sizes[plate_size]/2 - 30.1625,0,3.5]) cylinder(r1=2, r2=3.3, h=3);
     translate([height_sizes[plate_size]/2 - 30.1625,0,6.0]) cylinder(r=3.6, h=10);
}
...
if (plate_width == 1) {
	difference() {
		plate();
		translate([0,0,-3-min(screwhole_recess,2.99)]) plate_inner();
		plate1();
	}
	union() {
		plate1_solid();
